...
class MatrixVectors(object):
    ...
    def move_at(self, x, y, rel_x, rel_y):
        ...
        REL_X = 100 * rel_x / self._case_width
        REL_Y = 100 * rel_y / self._case_height
...
class Renderer(object):
    ...
    def init(self):
        pygame.init()
        pygame.display.set_mode((400, 400), OPENGL|DOUBLEBUF)
...
def main(args):
    ...
    nats = LazyNavier(NavierStrokesCuda, size=size)

    mdensity = MatrixDensity(nats.me.density_prev, 0, 0, 400, 400)
    mvectors = MatrixVectors(nats.me.u_prev, nats.me.v_prev,
                             0, 0, 400, 400)
